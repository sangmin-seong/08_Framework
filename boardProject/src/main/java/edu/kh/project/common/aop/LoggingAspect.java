...
public class LoggingAspect {
	...
	public void beforeController(JoinPoint jp) {
		...
		// 로그인이 되어있는 경우
		// 로그인한 회원의 회원번호, 이메일 추가
		Object loginMember 
			= req.getSession().getAttribute("loginMember");
		
		if(loginMember != null) { // login 데이터가 session에 있는 경우
			Member member = (Member)loginMember; // 다운캐스팅
	 ...
	 *  
	 *  [ 주의 사항 ]
	 *  1) @Around 사용 시 항상 반환형은 Object타입
	 *  	(다른 것은 void)
	 *  2) 메서드 종료 시 proceed() 메서드 호출 값을 반환해야함!
	 */
	...
	public Object aroundServiceImpl(ProceedingJoinPoint pjp) throws Throwable {
		...
		Object obj = pjp.proceed(); // 전/후를 나누는 기준
