...
public class ImageDeletScheduling {
	...
	// 0초 시작, 20초가 지날 때 마다 수행(0, 20, 40 초 동작)
	@Scheduled(cron = "0/20 * * * * *")
	public void imageDelete() {
		...
		//		- 두 결과를 UNION 해서 하나의 SELECT 결과로 반환
		List<String> dbFileNameList	= service.getDbFileNameList();
		
//		if(dbFileNameList.isEmpty()) return;
		
		// 2) 서버에 저장된 파일명 모두얻어오기
