...
//		System.out.println(inputBoard.getBoardNo());
		...
		// 여러 행 한번에 삽입 후 삽입된 행의 갯수 반환받기
		int insertRows = mapper.insertUploadList(uploadList);
		
		// insert 된 행의 갯수와 uploadList의 개수가 같지 않은 경우
		...
		try {
			...
			e.printStackTrace();
			throw new FileUploadFailException(); // 사용자 정의 예외 던짐 -> rollback 실행
		}
