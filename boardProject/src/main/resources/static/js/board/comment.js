...
const selectCommentList = () =>{
  // boardNo : 게시글 번호(boardDetail 전역 변수)
  ...
  .then(html =>{
    ...
    // 타임리프가 해석된 html 코드를
    // s.comment-list-area의 내용으로 대입 후 html 코드 해석
    ...
    addEventDeleteComment(); // 삭제 버튼 클릭 이벤트 추가
...
/** 비동기로 댓글 등록 함수(AJAX)
 * @param parentCommentNo : 부모 댓글 번호(없으면 undifined)
 */
...
*/
  ...
  const showChildComment = (btn) => {
    ...
    //클릭된 답글 버튼이 속해 있는 댓글(li) 요소 찾기
    // closest("태그") : 부모 중 가장 가까운 태그 찾기
    const li = btn.closest("li");

    // 답글이 작성되는 댓글(부모댓글) 번호 얻어오기
  ...
  const showUpdateComment = (btn) => {
    ...
    if(temp != null){ // 이미 열려있는 수정화면이 있을 경우
      if(confirm("수정중인 댓글이 있습니다."
        ...
        const childCommentBtn  = beforeCommentRow.querySelector(".child-comment-btn"); // 답글 버튼
        const updateCommentBtn = beforeCommentRow.querySelector(".update-comment-btn"); // 수정 버튼
        const deleteCommentBtn = beforeCommentRow.querySelector(".delete-comment-btn"); // 삭제 버튼

        childCommentBtn.addEventListener("click",  () => showChildComment(childCommentBtn) ); // 답글 버튼 이벤트 추가
        updateCommentBtn.addEventListener("click", () => showUpdateComment(updateCommentBtn)); // 수정 버튼 이벤트 추가
        deleteCommentBtn.addEventListener("click", () => deleteComment(deleteCommentBtn)); // 삭제 버튼 이벤트 추가
      }else{
        return;
      }
    }
    ...
    */
    ...
    cancelBtn.addEventListener("click", () => {
      ...
      const updateCommentBtn
            = beforeCommentRow.querySelector(".update-comment-btn"); // 수정 버튼
      ...
      const deleteCommentBtn
            = beforeCommentRow.querySelector(".delete-comment-btn"); // 삭제 버튼
...
addComment.addEventListener("click", () =>{
  ...
  // 조건 1) 로그인 여부 검사(boardDetail.html의 loginCheck 이용)
    if(loginCheck == false){
      alert("로그인 후 이용해주세요");
      return;
    }
  ...
  if(commentContent.value.trim().length === 0){
    alert("댓글 내용 작성 후 등록 버튼을 클릭해주세요");
    return;
  }
  
  // 조건 1, 2) 통과 시 댓글 등록 함수 호출
  insertComment();
...
const addEventUpdateComment = () => {
  ...
  btns.forEach(btn => {
    btn.addEventListener("click", () => {
      showUpdateComment(btn); // 수정버튼 클릭 시 showUpdateComment() 호출
    })
  })
}

/* 화면 로딩이 끝났을 때(화면 코드 해석 완료 후) */
document.addEventListener("DOMContentLoaded", () => {
  addEventChildComment(); // 답글 버튼 이벤트 추가
  addEventDeleteComment(); // 삭제 버튼 이벤트 추가
  addEventUpdateComment(); // 수정 버튼에 이벤트 추가
});
