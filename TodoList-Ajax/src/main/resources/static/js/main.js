  ...
  function selectTodoList(){
    ...
    .then(result => {
      ...
      for(let todo of todoList){
        ...
        // 5) tr 요소를 만들어 1,2,3,4 에서 만든 요소 자식으로 추가하는 코드
  ...
  function selectTodo(url){
    ...
    .then(response => {
      if(response.ok){
        return response.json(); // response.text() + JSON.parse() 합친 메서드
                               // 문자열 형태 변환 + JSON -> JS Object변환
      }
  ...
  changeComplete.addEventListener("click", () => {
    ...
    */
    fetch("/todo/todoComplete",{
      ...
      // 제출되는 데이터는 json 형태임을 정의
      body : todoNo // todoNo 번호
    })
    .then(response => {
      if(response.ok) return response.text();

      throw new Error("완료여부 변경 실패" + response.status);
    })
    .then(result => {
      console.log(result);
...
updateView.addEventListener("click", ()=>{
  ...
  updateLayer.classList.remove("popup-hidden"); // 수정레이어 보이기
...
updateCancel.addEventListener("click", ()=>{
  ...
  updateLayer.classList.add("popup-hidden"); // 수정레이어 숨기기
});
...
updateBtn.addEventListener("click", ()=>{
  ...
  todo.todoNo = updateBtn.dataset.todoNo;
  todo.todoTitle = document.querySelector("#updateTitle").value;
  todo.todoDetail = document.querySelector("#updateContent").value;

  fetch("/todo/todoUpdate", {
    method : "PUT",
    headers : {"Content-Type" : "application/json"},
    body : JSON.stringify(todo)
    // todo 객체를 JSON 문자열 형태로 변환해서 제출
  })
  .then(response => {
